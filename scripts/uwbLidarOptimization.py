...
def listener():
    global initUWB, uwbMsg, updateUWBLocalization, lidar_offsetx, lidar_offsety, \
        updateLidar, lidarData, resolution, odoMsg, config
    ...
    rospy.Subscriber(lidarTopic, LaserScan, robot_lidar_msg_callback)
    
    if isSimulation == True:
        odoTopic = config.default.robot_encoder.simulateMessageTopic
        rospy.Subscriber(odoTopic, Odometry, robot_odo_msg_callback )
    else:
    ...
    while not rospy.is_shutdown():
        ...
        if initTheta == False:
            ...
            Theta = uwbMsg.theta - angularOffset
            if Theta < 0:
                Theta = Theta + 2 * np.pi
            elif Theta > 2*np.pi:
                Theta = Theta - 2*np.pi
            ...
            if np.sqrt( (prePos[0]-uwbMsg.robotPosx)**2 + (uwbMsg.robotPosy - prePos[1])**2 ) * resolution < 100:
                start_time = time.time()
                rate.sleep()
                continue
            else:
        ...
        if isSimulation:
            vel = np.sqrt(odoMsg.twist.twist.linear.x**2 + odoMsg.twist.twist.linear.x**2)
            omega = odoMsg.twist.twist.angular.z
            l_vel = (vel - 0.5 * omega * fWheelDistance) * 100
            r_vel = (vel + 0.5 * omega * fWheelDistance) * 100
        else:
            r_vel = odoMsg.right_velocity
            l_vel = odoMsg.left_velocity
        ...
        [updateFlag, optPos] = pf.doParticleFiltering(mapMatrix5, mapMatrixSave, robotState, obsToRobot, \
             r_vel, l_vel, mappinginforMsg, mappingPub)
